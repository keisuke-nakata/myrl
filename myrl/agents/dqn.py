...
class DQNAgent:
    ...
    def build(self):
        ...
        self.recorder = StandardRecorder(os.path.join(result_dir, 'history.csv'))
        self.recorder.start()

        self.eval_recorder = StandardRecorder(os.path.join(result_dir, 'eval_history.csv'))
        ...
        plot_graph(self.network, os.path.join(result_dir, 'graph.png'))
        if self.device >= 0:
