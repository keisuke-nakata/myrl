...
class Actor:
    ...
    #     is_random = self.explorer(step)
    #     if is_random:
    #         action = self.env.action_space.sample()
    #     else:
    #         action = self.policy(self.state)
    #     return is_random, action
    ...
    def _reset(self):
        ...
        self.is_done = False
        # self.episode_processed_obses.append(self.obs_preprocessor(observation, None))
        ...
        for _ in range(n_random_actions):
            ...
            self.episode_obses.append(observation)
            # reward, done = self._repeat_action(NOOP_ACTION)
        assert not done
    ...
    def _repeat_action(self, action):
        ...
        for repeat in range(1, self.n_action_repeat + 1):
            ...
            if done:
                # for _ in range(self.n_action_repeat - repeat):
                #     self.episode_processed_obses.append(self.episode_processed_obses[-1].copy())
                break
        self.episode_processed_obses.append(self.obs_preprocessor(self.episode_obses[-1], self.episode_obses[-2]))
        return reward, done
    ...
    def act(self, step):
        ...
        self.is_done = done
        return state, action, reward, next_state, done, is_random
